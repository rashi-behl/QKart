  ...
  const performAPICall = async () => {
    ...
    try{
      ...
      if (e.response && e.response.status ===500) {
        ...
        enqueueSnackbar("Could not fetch products. Check that backend is running, reachable and returns valid JSON.",
        {variant:"error"});
      }
    }
  };
  ...
  const performSearch = async (text) => {
    try{
      ...
      if(e.response) {
        ...
        enqueueSnackbar(
          "Could not fetch products. Check that backend is running, reachable and returns valid JSON.",
          {variant:"error"}
        );
      }
    }
